...
import (
	...
	"image/png"
	"net/http"
	getCaptcha "BubbleInn/web/proto/getCaptcha"
)
...
func GetSession(ctx *gin.Context){
	...
	ctx.JSON(http.StatusOK,resp) //json的序列化操作
...
//获取图片验证码信息
//发起微服务调用
func GetTmageCd(ctx *gin.Context){
	uuid := ctx.Param("uuid")
	//指定consul服务发现
...
//	)
//	//初始化客户端
//	microClient := userMicro.NewUsersService("go.micro.service.users", consulService.Client())
//
//	//调用远程函数
...
//	//fmt.Println(mobile,pwd,sms_code)//直接打印是空值，因为他只能获取form表单
//	var regData struct {
...
//
//	// 初始化客户端
//	microClient := userMicro.NewUsersService("go.micro.service.users", microService.Client())
//
//	// 调用远程函数
...
//	//获取数据。 因为数据来自  Request Payload , 所以：需要通过“Bind”来获取输入数据
...
//	// 获取图片文件, 静态文件对象
//	file, _ := ctx.FormFile("avatar") //返回一个文件指针
//	// 上传文件到项目中
